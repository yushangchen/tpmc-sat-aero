    ...
    for alt in altitude_list:
        for yaw in yaw_list:
            for n_particles in particle_list:
                case_name = f"cube_alt{alt:g}_yaw{yaw:g}_np{n_particles}"
...
def main():
    ...
    manager = BatchCaseManager(work_dir="runs_v2")
    ...
    print()
    print(f"Batch finished. New completed cases: {len(results)}")

    try:
        generate_standard_plots(
            batch_csv_path="runs_v2/batch_results.csv",
            output_dir="runs_v2/plots",
        )
